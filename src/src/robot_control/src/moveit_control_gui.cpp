...
public:
    MoveItControlGui(rclcpp::Node::SharedPtr node)
        ...
        QPushButton *move_l1l2_90deg_x_btn = new QPushButton("Link1-Link2 at 90° (X)", this);
        QPushButton *move_l1l2_90deg_y_btn = new QPushButton("Link1-Link2 at 90° (Y)", this);
        ...
        layout->addWidget(move_l1l2_90deg_x_btn);
        layout->addWidget(move_l1l2_90deg_y_btn);
        ...
        connect(move_l1l2_90deg_x_btn, &QPushButton::clicked, this, &MoveItControlGui::moveToL1L2_90Degree_x_Position);
        connect(move_l1l2_90deg_y_btn, &QPushButton::clicked, this, &MoveItControlGui::moveToL1L2_90Degree_y_Position);
...
private:
    ...
    // Home position using cartesian coordinates
    ...
    void moveToJointPosition(const std::vector<double>& joint_angles) {
        // Définir les valeurs des joints (en radians)
        std::vector<std::string> joint_names = {
            "R0_Yaw",    // Rotation base (yaw)
            "R1_Pitch",  // Angle entre link1 et link2 (pitch)
            "R2_Pitch",  // Angle entre link2 et link3 (pitch)
            "R3_Yaw",    // Rotation link3-link4 (yaw) 
            "R4_Pitch"   // Angle entre link4 et link5 (pitch)
        };
        
        if (joint_angles.size() != joint_names.size()) {
            RCLCPP_ERROR(node_->get_logger(), "Number of joint angles doesn't match number of joints!");
            status_label_->setText("Status: Joint angle count mismatch!");
            return;
        }
        
        // Utiliser setJointValueTarget au lieu de setPoseTarget
        move_group_interface_.setJointValueTarget(joint_names, joint_angles);
        
        // Planifier et exécuter le mouvement
        moveit::planning_interface::MoveGroupInterface::Plan my_plan;
        if (move_group_interface_.plan(my_plan) == moveit::core::MoveItErrorCode::SUCCESS) {
            RCLCPP_INFO(node_->get_logger(), "Joint plan successful! Executing joint movement...");
            move_group_interface_.execute(my_plan);
            status_label_->setText("Status: Moved to joint position successfully");
        } else {
            RCLCPP_ERROR(node_->get_logger(), "Joint planning failed!");
            status_label_->setText("Status: Joint planning failed!");
        }
    }

    // Position avec angle de 90° entre link1 et link2 (mouvement en Y)
    void moveToL1L2_90Degree_y_Position() {
        RCLCPP_INFO(node_->get_logger(), "Moving to Link1-Link2 90° position (Y axis)...");
        std::vector<double> joint_angles = {
            0.0,        // R0_Yaw : 0°
            -M_PI/2,    // R1_Pitch : -90° (pour lever le bras)
            0.0,        // R2_Pitch : 0°  
            0.0,        // R3_Yaw : 0°
            0.0         // R4_Pitch : 0°
        };
    ...
    // Position avec angle de 90° entre link1 et link2 (mouvement en X)
    void moveToL1L2_90Degree_x_Position() {
        RCLCPP_INFO(node_->get_logger(), "Moving to Link1-Link2 90° position (X axis)...");
        std::vector<double> joint_angles = {
            M_PI/2,     // R0_Yaw : 90° (rotation de la base)
            -M_PI/2,    // R1_Pitch : -90° (pour lever le bras)
            0.0,        // R2_Pitch : 0°  
            0.0,        // R3_Yaw : 0°
            0.0         // R4_Pitch : 0°
