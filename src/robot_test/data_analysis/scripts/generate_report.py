...
def process_logs():
    ...
    for folder in os.listdir(LOGS_DIR):
        ...
        if (
            not os.path.exists(error_file)
            or not os.path.exists(detection_file)
            or not os.path.exists(info_file)
        ):
        ...
        analyze_and_generate_report(
            log_folder_path, error_file, detection_file, info_file, report_path
        )


def analyze_and_generate_report(
    folder_path, error_file, detection_file, info_file, report_path
):
    # Load data
    ...
    for marker_id in errors_df["marker_id"].unique():
        ...
        errors = (
            marker_data["dx"] ** 2 + marker_data["dy"] ** 2 + marker_data["dz"] ** 2
        ) ** 0.5
    ...
    if overall_mean is not None:
        ...
        for col in overall_mean.index:
            pdf.cell(
                200,
                10,
                txt=f"    {col}: Mean={overall_mean[col]:.4f}, Std={overall_std[col]:.4f}",
                ln=True,
            )
    pdf.cell(200, 10, txt="Detection Analysis:", ln=True, align="L")
    pdf.cell(
        200, 10, txt=f"Mean Number of Detected Markers: {detection_mean:.4f}", ln=True
    )
    ...
    for axis, path in orientation_plot_paths.items():
        ...
        pdf.cell(
            200,
            10,
            txt=f"Orientation Errors ({axis.upper()}) Over Time",
            ln=True,
            align="C",
        )
